...
The One-IP table records the physical connections for all IP addresses in your workspace. It is retrieved during the Layer 2 topology discovery. You can use the One-IP table to troubleshoot any Layer 2 connection issues.This API is used to get the corresponding OneIPTbale result by IP, Lan Segment, MAC Address, Switch Name, and DNS Name.The response of this API will return a list in JSON format.
...
If user only provide the input values of "beginIndex" and "count", API will return One-IP Table with items number equal to "count" values start from "beginIndex". The default maximum value of "count" is 100,000. So if the input value of "count" greater than 100,000, then API would return an error. And if start from beginIndex to the end of table, there are no enough count items, API will return the rest of items.
...
3. For each query parameters, it doesn't support to set value as "None" or ""(enmpty string/integer).
## Detail Information
...
| count | int  | Count number of returned data, API will return the total number items of "count" value. The Valid range of ‘count’ is between 1 and 100,000; 100,000 is included. If the input value of "count" greater than 100,000, then API would return an error. If the total number of items which start from "beginIndex" to the end of table are less than "count" value, API will return the rest of items. This parameter works for all scenario.<br> ***Note:*** If user insert the beginindex as 1 and count as 100,002 which means the total number will greater than 100,000. Then user would get an error and without any result returned. | 100,000|
